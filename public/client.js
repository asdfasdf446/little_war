...
socket.on('state', (state) => {
    ...
    // 这里调用了 updateScoreboard，下面必须定义它
...
if (!isMobile) {
    window.addEventListener('keydown', e => {
        ...
        if(e.code === 'Space') input.skill = true;
    });
...
function updateScoreboard(players) {
    ...
    let html = '';
    list.forEach(p => {
        let skillReady = p.skillCdCurrent <= 0 ? '<span style="color:#0f0">READY</span>' : `<span style="color:#555">${Math.ceil(p.skillCdCurrent/60)}s</span>`;
        let typeName = p.type === 'RIFLE' ? '步枪' : (p.type === 'SNIPER' ? '狙击' : '喷子');
        let name = p.ip.split('.').pop();
        let style = (p.id === myId) ? 'font-weight:bold; color:#fff' : 'color:#aaa';
        html += `<div style="${style}">[${typeName}] ${name} : ${p.score} | [${skillReady}]</div>`;
    });
